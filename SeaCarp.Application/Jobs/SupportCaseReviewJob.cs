...
public class SupportCaseReviewJob(IServiceScopeFactory scopeFactory) : BackgroundService
{
    ...
    protected override async Task ExecuteAsync(CancellationToken stoppingToken)
    {
        while (!stoppingToken.IsCancellationRequested)
        {
            ...
            var logService = scope.ServiceProvider.GetRequiredService<ILogService<SupportCaseReviewJob>>();
                        ...
                        try
                        {
                            ...
                            foreach (var supportCase in supportCases)
                            {
                                try
                                {
                                    ...
                                    logService.Information($"Case {supportCase.CaseNumber} was assigned to {user.Username}");
                                }
                                catch (Exception ex)
                                {
                                    logService.Error($"Stacktrace: {ex.StackTrace}");
