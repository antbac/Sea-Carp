...
public class User
{
    ...
    public bool IsCaseOfficer { get; internal set; }
    public List<Order> Orders { get; internal set; } = [];

    public AuthenticationLevel AuthenticationLevel => this switch
    {
        { IsAdmin: true } => AuthenticationLevel.Admin,
        { IsCaseOfficer: true } => AuthenticationLevel.CaseOfficer,
        _ => AuthenticationLevel.User
    };
    ...
    public User PromoteToCaseOfficer()
    {
        if (IsCaseOfficer)
        {
            throw new InvalidOperationException("User is already a case officer.");
        }
    ...
    public User DemoteFromCaseOfficer()
    {
        if (!IsCaseOfficer)
        {
            throw new InvalidOperationException("Cannot demote a user who is not a case officer.");
        }
