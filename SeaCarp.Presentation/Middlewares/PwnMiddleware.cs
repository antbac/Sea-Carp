...
public static class PwnMiddleware
{
    ...
    public static IApplicationBuilder UsePwnMiddleware(this IApplicationBuilder app)
    {
        ...
        {
            var sitePwnedBy = GetEnvVar("SYSTEM_PWNER");
            if (string.IsNullOrWhiteSpace(sitePwnedBy))
            {
                await next(context);
                return;
            }
            ...
            if (string.IsNullOrWhiteSpace(_pwnMessage))
            {
                ...
                _pwnMessage = content.Replace("[HACKER_USERNAME]", sitePwnedBy);
            }
    ...
    private static string GetEnvVar(string variableName)
    {
        ...
        try
        {
            ...
            psi.ArgumentList.Add($"echo ${variableName}");
